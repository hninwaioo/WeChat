...
class AddNewPostBloc extends ChangeNotifier{
  ...
  AddNewPostBloc({int? newsfeedId}){
    ...
    debugPrint("NewsFeedID=>${newsfeedId}");
    if(newsfeedId != null){
      isInEditMode = true;
  ...
  Future onTapAddNewPost(){
    if(newPostDescription.isEmpty){
      ...
      debugPrint("newPostDescription=>${newPostDescription}");
      ...
      if(isInEditMode){
        return _editNewsFeedPost().then((value){
          isLoading = false;
          debugPrint("isInEditMode=>${isInEditMode}");
          _notifySafety();
  ...
  Future<dynamic> _editNewsFeedPost(){
    ...
    if(mNewsFeed != null){
      return _model.editPost(mNewsFeed!,chosenImageFileList);
    }else {
      return Future.error("Error");
    }
  }
  ...
  Future<void> _createNewNewsfeedPost(){
    return _model.addNewPost(newPostDescription,chosenImageFileList);
  }
